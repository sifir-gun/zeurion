...
3. **Design** : proposer 3-4 directions visuelles concrètes (fond, accent, typographie, une ligne de justification chacune) et faire choisir Guney avant d'implémenter. Éviter les esthétiques génériques (Inter partout, dégradés violets) ; viser un caractère propre à la niche de la venture. Contraste AA minimum, lisible en clair et sombre si le thème double existe.
...
7. **Mise en ligne et contrôle** : déploiement, vérification du checkout en mode test Stripe de bout en bout, puis contrôle qualité (étape 8).
